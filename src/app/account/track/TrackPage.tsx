...
interface TimelineItemProps {
  ...
  time: string;
  icon: LucideIcon;
  color: 'amber' | 'blue' | 'slate' | 'emerald' | 'red';
  isLast?: boolean;
}
...
function ActivityMonitor() {
  const { user } = useAuth();
  const [stats, setStats] = useState({ active: 0, pending: 0, action: 0 });
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      if (!user?.id) return;
      try {
        const { authenticatedFetch, getSupabaseToken } = await import('@/utils/supabaseTokenHelper');
        let token: string | null = null;
        try { token = await getSupabaseToken(); } catch (err) { token = null; }

        let res: Response;
        if (token) {
          // Fetch all to calculate stats (limit removed or set high)
          res = await authenticatedFetch(`/api/auth/applications?limit=100`);
        } else {
          const profileId: string | null = user?.id || (typeof window !== 'undefined' ? localStorage.getItem('lead_user_id') : null);
          if (!profileId) return;
          res = await fetch(`/api/auth/applications?profileId=${encodeURIComponent(profileId)}&limit=100`);
        }
        ...
        if (res.ok && json?.success && mounted) {
          ...
          const active = apps.filter(a => ['approved', 'disbursed'].includes(a.status?.toLowerCase())).length;
          const pending = apps.filter(a => ['pending', 'new', 'in_review'].includes(a.status?.toLowerCase())).length;
          const action = apps.filter(a => ['rejected', 'on_hold', 'more_info'].includes(a.status?.toLowerCase())).length;

          setStats({ active, pending, action });
        }
      } catch (e) {
  ...
  if (loading) return <div className="text-xs text-slate-400">Loading stats...</div>;

  return (
    <div className="grid grid-cols-3 gap-4">
      <MetricCard label="Active" value={stats.active} color="emerald" icon={Activity} />
      <MetricCard label="Pending" value={stats.pending} color="amber" icon={Clock} />
      <MetricCard label="Action" value={stats.action} color="rose" icon={AlertCircle} />
    </div>
  );
}
...
function UserApps() {
  ...
  useEffect(() => {
    ...
    const fetchApps = async () => {
      ...
      try {
        ...
        let res: Response;
        if (token) {
          res = await authenticatedFetch(`/api/auth/applications?limit=10`);
        } else {
          const profileId: string | null = user?.id || (typeof window !== 'undefined' ? localStorage.getItem('lead_user_id') : null);
          if (!profileId) {
            console.warn('No profileId available; skipping applications fetch');
            return;
          }
          res = await fetch(`/api/auth/applications?profileId=${encodeURIComponent(profileId)}&limit=10`);
        }

        const json = await res.json();
        if (res.ok && json?.success) {
          if (mounted) setApps(json.data || []);
        } else {
          console.error('Failed to fetch user applications', json?.error || res.statusText);
        }
      } catch (e) {
  ...
  if (loading) return <div className="p-6 text-center text-sm text-slate-500">Loading applications...</div>;
    ...
    return (
      <div className="p-12 text-center flex flex-col items-center justify-center">
        ...
        <p className="text-slate-500 text-sm max-w-xs mx-auto mb-6">
          You haven't submitted any loan applications recently. Start a new application to see it here.
        </p>
        <Link 
          href="/loans/personal-loan"
          className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-xl transition-colors shadow-lg shadow-blue-200"
        >
          Apply Now
        </Link>
      </div>
  ...
  return (
    <div className="p-0">
      {apps.map((a, index) => {
        ...
        let color: 'amber' | 'blue' | 'emerald' | 'red' | 'slate' = 'blue';
        const s = a.status?.toLowerCase();
        if (s === 'approved') color = 'emerald';
        else if (s === 'rejected') color = 'red';
        else if (s === 'pending') color = 'amber';

        return (
          <TimelineItem 
            key={a.id}
            title={`${a.product_type} (${a.product_category})`}
...
function TimelineItem({ title, id, status, time, icon: Icon, color, isLast }: TimelineItemProps) {
  const colorMap = {
  ...
  return (
    <div className="relative pl-6 md:pl-8 pr-6 py-6 group hover:bg-slate-50 transition-colors">
      ...
      <div className="flex gap-4 md:gap-5">
        {/* Icon Node */}
        <div className={`relative z-10 w-10 h-10 md:w-12 md:h-12 rounded-full ${colorMap[color] || colorMap.slate} flex items-center justify-center shrink-0 shadow-sm border-2 border-white`}>
          <Icon className="w-5 h-5" />
        </div>
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-slate-600 mb-2">
            <span className="font-medium text-slate-800 bg-white border border-slate-200 px-1.5 py-0.5 rounded-md text-xs mr-2 font-mono">{id}</span>
            Status: <span className={`font-bold capitalize ${color === 'amber' ? 'text-amber-600' : color === 'emerald' ? 'text-emerald-600' : color === 'red' ? 'text-red-600' : 'text-slate-700'}`}>{status}</span>
          </p>
        </div>
