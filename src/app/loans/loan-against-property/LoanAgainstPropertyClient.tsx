...
import { 
  ...
  ArrowRight, Building, Wallet, Shield, Clock, TrendingUp,
      ...
      <div className="bg-slate-900 text-white p-6 rounded-2xl text-center shadow-lg">
        ...
        <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-800">
            ...
            <div>
              ...
              <p className="text-sm font-bold text-indigo-400">₹{totalAmount.toLocaleString()}</p>
            </div>
        </div>
      </div>
      ...
      <div className="space-y-6 bg-white md:bg-transparent p-4 md:p-0 rounded-xl border md:border-none border-slate-200">
        ...
        <div>
          ...
          <div className="flex gap-2 mt-3 overflow-x-auto pb-1 scrollbar-hide">
             {[2500000, 5000000, 10000000, 20000000].map(val => (
               <button 
                 key={val}
                 onClick={() => setLoanAmount(val)}
                 className={`px-3 py-1 text-xs font-medium rounded-full border transition-all whitespace-nowrap ${loanAmount === val ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-600 hover:border-indigo-200'}`}
               >
...
const LoanAgainstPropertyClient = () => {
  // UI State
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20 lg:pb-0 text-slate-900">
      ...
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 px-4 md:px-8 h-16 flex items-center justify-between">
        <div className="max-w-7xl mx-auto w-full flex items-center justify-between">
          ...
          <h1 className="text-base md:text-lg font-bold text-slate-900">Loan Against Property</h1>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 pt-6">
        ...
        <section id="overview" className="mb-8">
          <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[2rem] p-6 md:p-10 text-white shadow-xl shadow-indigo-200/50 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              <div className="max-w-2xl">
                ...
                <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold leading-tight mb-4">
                  Unlock the value of <br className="hidden md:block"/> <span className="text-indigo-100">your property.</span>
                </h2>
                <p className="text-indigo-50 text-sm md:text-lg mb-6 leading-relaxed max-w-lg">
                  Get up to ₹5 Crore for business or personal needs. 
                  Lowest interest rates. Retain ownership.
                </p>
                ...
                <div className="flex flex-wrap gap-3 text-xs md:text-sm font-medium">
                  <div className="flex items-center gap-2 bg-black/20 px-4 py-2 rounded-full backdrop-blur-md">
                    <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" /> High LTV
                  </div>
                  <div className="flex items-center gap-2 bg-black/20 px-4 py-2 rounded-full backdrop-blur-md">
                    <Check className="w-4 h-4 text-green-400" /> Overdraft Facility
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-7 xl:col-span-8 space-y-16">
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div>
                ...
                <p className="text-sm md:text-base text-slate-600 leading-relaxed text-justify">
                  A Loan Against Property (LAP) allows you to leverage your residential or commercial property to get high-value funds at lower interest rates. The money can be used for business expansion, weddings, medical emergencies, or education.
                </p>
              </div>

              {/* Why Choose LAP Box */}
              <div className="bg-indigo-50/80 p-6 rounded-2xl border border-indigo-100">
                  <h3 className="text-sm font-bold text-indigo-900 mb-4 uppercase tracking-wide">Key Highlights</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                      ...
                      <div key={i} className="bg-white p-4 rounded-xl border border-indigo-50 shadow-sm flex flex-col justify-center">
                        <p className="text-[10px] md:text-xs text-slate-400 font-bold uppercase">{stat.title}</p>
                        <p className="text-sm md:text-base font-bold text-indigo-700 mt-1">{stat.val}</p>
                      </div>
                    ))}
                  </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {features.map((feat) => (
                  <div key={feat.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow flex items-start gap-4">
                    <div className="w-12 h-12 bg-slate-50 rounded-full flex items-center justify-center shadow-inner shrink-0 text-indigo-600">
            ...
            <div id="faqs" className="scroll-mt-32 border-t border-slate-200 pt-8">
              ...
              <div className="space-y-3">
                {faqs.map((item, i) => (
                  <div key={i} className="border border-slate-200 rounded-xl overflow-hidden bg-white hover:border-indigo-300 transition-colors">
                    <button 
                      onClick={() => toggleFaq(i)}
                      className="w-full flex justify-between items-center p-5 text-left"
                    >
                      <span className="text-sm md:text-base font-semibold text-slate-800 pr-4">{item.q}</span>
                      <ChevronDown className={`w-5 h-5 text-slate-400 transition-transform flex-shrink-0 ${activeFaq === i ? 'rotate-180 text-indigo-600' : ''}`} />
                    </button>
                    {activeFaq === i && (
...
export default LoanAgainstPropertyClient;