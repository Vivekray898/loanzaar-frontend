...
interface CarRefinanceFormProps {
  isOpen: boolean;
  onClose: () => void;
  loanType?: string;
}

export default function CarRefinanceForm({ 
  isOpen, 
  onClose, 
  loanType = 'Car Refinance' 
}: CarRefinanceFormProps) {
  ...
  const handleFinalSubmit = async () => {
    if (!formData.vehicleModel || !formData.outstandingAmount || !formData.pincode) {
      ...
      <div className="w-full max-w-lg bg-white md:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col animate-in slide-in-from-bottom-10 duration-300">
        ...
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10">
          <div>
            ...
            <p className="text-xs text-slate-500">Lower your EMI today</p>
          </div>
          <button 
            onClick={onClose} 
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div className="text-center mb-6">
                ...
                <p className="text-sm text-slate-500">We need this to discuss your refinance options.</p>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-5">
              <div className="text-center mb-6">
                ...
                <h4 className="text-xl font-bold text-slate-900">Loan Details</h4>
                <p className="text-sm text-slate-500">Details about your existing loan.</p>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 ml-1">Vehicle Model</label>
                <input 
                  name="vehicleModel" 
                  value={formData.vehicleModel} 
                  onChange={handleInput} 
                  placeholder="e.g. Hyundai Creta 2019" 
                  className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-red-500/20 focus:border-red-500 outline-none transition-all font-medium"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 ml-1">Current Lender</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 ml-1">Mfg Year</label>
                  <div className="relative">
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div>
                   <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 ml-1">Outstanding Amt</label>
                   <div className="relative">
          ...
          {step === 3 && (
            <div className="text-center py-10 animate-in zoom-in duration-300">
              ...
              <p className="text-slate-500 mb-8 max-w-xs mx-auto leading-relaxed">
                Thank you, <b>{formData.fullName}</b>. Our refinance expert will call you on <b>{formData.mobile}</b> shortly.
              </p>
              <button 
                onClick={onClose} 
