...
import { X, User, Shield, Check, Loader2, MapPin, Truck, Home, Plane } from 'lucide-react';
...
interface FormData {
  ...
  assetValue: string;
  registrationNumber?: string; // Optional, mostly for vehicles
}
...
export default function GeneralInsuranceForm({ 
  ...
  const handleFinalSubmit = async () => {
    if (!formData.insuranceType || !formData.pincode || !formData.assetValue) {
    ...
    try {
      const payload = {
        ...
        source: 'website',
        metadata: {
          insuranceType: formData.insuranceType, // e.g., Car, Home, Travel
      ...
      console.log('📤 Submitting General Insurance application...', { mobile: payload.mobile });
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-medium"
                  />
                </div>
              </div>
              ...
              <button 
                ...
                className="w-full mt-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-indigo-200 active:scale-[0.98] transition-all"
              >
                Continue
              </button>
            </div>
          ...
          {step === 2 && (
            <div className="space-y-5">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full pl-10 pr-3 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all font-medium"
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : (!!TURNSTILE_SITE_KEY && !captchaToken ? 'Complete Security Check' : 'Get Quote')}
                </button>
              </div>
            </div>
          )}
