...
import { X, User, Heart, Check, Loader2, MapPin, Shield, Activity } from 'lucide-react';
...
export default function HealthInsuranceForm({ 
  ...
  const handleFinalSubmit = async () => {
    if (!formData.pincode || !formData.age || !formData.coverageAmount) {
      alert('Please fill in all required health details.');
      return;
    }
    ...
    try {
      const payload = {
        ...
        loanType: planType, // Saving as loanType for consistency in DB schema
      ...
      console.log('📤 Submitting Health Insurance application...', { mobile: payload.mobile });

      const res = await submitApplication(submitPayload);
      
      if (res && res.success) {
        console.log('✅ Health application submitted:', res);
        setStep(3);
        setCaptchaToken(null);
      } else {
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-3 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium"
                  />
                </div>
              </div>
