...
export default function GoldLoanForm({ isOpen, onClose, loanType = 'Gold Loan' }: GoldLoanFormProps) {
  ...
  const handleFinalSubmit = async () => {
    if (!formData.goldWeight || !formData.purity || !formData.loanAmountDesired || !formData.pincode) {
      alert('Please fill required gold and contact fields.');
      return;
    }
      ...
      <div className="w-full max-w-lg bg-white md:rounded-3xl rounded-t-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col animate-in slide-in-from-bottom-10 duration-300">
        ...
        {step < 3 && (
          <div className="h-1 w-full bg-slate-100">
            <div className="h-full bg-amber-500 transition-all duration-500 ease-out" style={{ width: step === 1 ? '50%' : '100%' }} />
          </div>
        )}
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <input name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="As per documents" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all font-medium" />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input name="mobile" type="tel" maxLength={10} value={formData.mobile} onChange={handleInputChange} placeholder="99999 00000" className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all font-medium" />
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 ml-1">Email (optional)</label>
                <input name="email" type="email" value={formData.email} onChange={handleInputChange} placeholder="you@example.com" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all font-medium" />
              </div>

              <button onClick={handleNextStep} className="w-full mt-2 bg-amber-500 hover:bg-amber-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-amber-200 active:scale-[0.98] transition-all">Continue</button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-5">
              <div className="text-center mb-6">
                ...
                <h4 className="text-xl font-bold text-slate-900">Gold & Loan Details</h4>
                <p className="text-sm text-slate-500">Provide gold details to estimate loan value.</p>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 ml-1">Gold Weight (grams)</label>
              ...
              <div>
                ...
                <input name="loanAmountDesired" value={formData.loanAmountDesired} onChange={handleInputChange} placeholder="e.g. 50,000" className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all font-medium" />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 ml-1">Preferred Tenure</label>
                <select name="tenurePreference" value={formData.tenurePreference} onChange={handleInputChange} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all font-medium">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input name="pincode" type="number" maxLength={6} value={formData.pincode} onChange={handleInputChange} placeholder="e.g. 110001" className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-amber-500/20 focus:border-amber-500 outline-none transition-all font-medium" />
                </div>
              </div>
