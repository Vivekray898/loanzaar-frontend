        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            {!submitted ? (
              <>
                {/* Professional Header */}
                <div className="bg-gradient-to-r from-green-500 to-green-600 p-4 md:p-8 flex-shrink-0 rounded-t-2xl">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <h2 className="text-xl md:text-3xl font-bold text-white mb-1">Solar Loan Application</h2>
                      <p className="text-green-50">Go green financing • Quick approval • Minimal documentation</p>
                    </div>
                    <button onClick={closeModal} className="text-white hover:bg-green-500 p-2 rounded-lg transition">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-semibold text-white">Progress: Step {currentStep} of 3</span>
                      <span className="text-xs bg-green-400 text-green-900 px-3 py-1 rounded-full font-medium">{Math.round((currentStep / 3) * 100)}%</span>
                    </div>
                      ...
                      {[1, 2, 3].map((step) => (
                        <div key={step} className="flex-1 flex items-center">
                          <div className={`w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center font-semibold text-sm md:text-base transition-all ${
                            currentStep > step ? 'bg-white text-green-600' :
                            currentStep === step ? 'bg-white text-green-600 ring-2 ring-green-200' :
                            'bg-green-200 text-green-600'
                          }`}>
                            {currentStep > step ? '✓' : step}
                          </div>
                          <span className={`text-xs md:text-sm font-medium ml-2 ${
                            currentStep >= step ? 'text-white' : 'text-green-100'
                          }`}>
                            {step === 1 ? 'Personal' : step === 2 ? 'Solar Details' : 'Consent'}
                          </span>
                        </div>
                      ))}
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-4 md:p-8 space-y-6">
                  ...
                  {currentStep === 2 && (
                    <div className="space-y-6">
                      {/* Property & Installation Section */}
                      <div className="bg-green-50 rounded-lg p-4 md:p-6 border border-green-100">
                        <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-4 flex items-center">
                          <span className="text-2xl mr-2">🏠</span>
                          Property & Installation Details
                        </h3>
                        <div className="grid md:grid-cols-2 gap-4 md:gap-5">
                          {/* Property Type */}
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (fieldErrors.propertyType) setFieldErrors({ ...fieldErrors, propertyType: '' });
                              }}
                              className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                                fieldErrors.propertyType ? 'border-red-500 bg-red-50' : 'border-gray-300'
                              }`}
                            >
                            ...
                            {fieldErrors.propertyType && <p className="text-red-600 text-xs mt-1 flex items-center">❌ {fieldErrors.propertyType}</p>}
                          </div>
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (fieldErrors.ownershipType) setFieldErrors({ ...fieldErrors, ownershipType: '' });
                              }}
                              className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                                fieldErrors.ownershipType ? 'border-red-500 bg-red-50' : 'border-gray-300'
                              }`}
                            >
                            ...
                            {fieldErrors.ownershipType && <p className="text-red-600 text-xs mt-1 flex items-center">❌ {fieldErrors.ownershipType}</p>}
                          </div>

                          {/* Roof Type - Dropdown */}
                          <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-2">Roof Type / Area (Approx.) *</label>
                            <select
                              value={formData.roofTypeArea}
                              onChange={(e) => {
                                setFormData({ ...formData, roofTypeArea: e.target.value });
                                if (fieldErrors.roofTypeArea) setFieldErrors({ ...fieldErrors, roofTypeArea: '' });
                              }}
                              className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                                fieldErrors.roofTypeArea ? 'border-red-500 bg-red-50' : 'border-gray-300'
                              }`}
                            >
                              <option value="">Select roof type/area</option>
                              <option value="Concrete - <200 sq ft">Concrete - &lt;200 sq ft</option>
                              <option value="Concrete - 200-500 sq ft">Concrete - 200-500 sq ft</option>
                              <option value="Concrete - 500-1000 sq ft">Concrete - 500-1000 sq ft</option>
                              <option value="Concrete - >1000 sq ft">Concrete - &gt;1000 sq ft</option>
                              <option value="Steel Sheet - <200 sq ft">Steel Sheet - &lt;200 sq ft</option>
                              <option value="Steel Sheet - 200-500 sq ft">Steel Sheet - 200-500 sq ft</option>
                              <option value="Steel Sheet - >500 sq ft">Steel Sheet - &gt;500 sq ft</option>
                              <option value="Other Material">Other Material</option>
                            </select>
                            {fieldErrors.roofTypeArea && <p className="text-red-600 text-xs mt-1 flex items-center">❌ {fieldErrors.roofTypeArea}</p>}
                          </div>

                          {/* System Capacity - Dropdown */}
                          <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-2">Solar System Capacity (in kW) *</label>
                            <select
                              value={formData.systemCapacity}
                              onChange={(e) => {
                                setFormData({ ...formData, systemCapacity: e.target.value });
                                if (fieldErrors.systemCapacity) setFieldErrors({ ...fieldErrors, systemCapacity: '' });
                              }}
                              className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                                fieldErrors.systemCapacity ? 'border-red-500 bg-red-50' : 'border-gray-300'
                              }`}
                            >
                              ...
                              <option value="6-10 kW">6-10 kW</option>
                              <option value="10-20 kW">10-20 kW</option>
                              <option value=">20 kW">&gt;20 kW</option>
                            </select>
                            {fieldErrors.systemCapacity && <p className="text-red-600 text-xs mt-1 flex items-center">❌ {fieldErrors.systemCapacity}</p>}
                          </div>

                          {/* Installation Status */}
                          <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-2">Installation Status *</label>
                            <select
                              value={formData.installationStatus}
                              onChange={(e) => {
                                setFormData({ ...formData, installationStatus: e.target.value });
                                if (fieldErrors.installationStatus) setFieldErrors({ ...fieldErrors, installationStatus: '' });
                              }}
                              className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                                fieldErrors.installationStatus ? 'border-red-500 bg-red-50' : 'border-gray-300'
                              }`}
                            >
                            ...
                            {fieldErrors.installationStatus && <p className="text-red-600 text-xs mt-1 flex items-center">❌ {fieldErrors.installationStatus}</p>}
                          </div>

                          {/* Vendor Name (Optional) */}
                          <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-2">Solar Company / Vendor Name</label>
                            <input
                              type="text"
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (fieldErrors.projectCost) setFieldErrors({ ...fieldErrors, projectCost: '' });
                              }}
                              className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                                fieldErrors.projectCost ? 'border-red-500 bg-red-50' : 'border-gray-300'
                              }`}
                            >
                              <option value="">Select project cost range</option>
                            ...
                            {fieldErrors.projectCost && <p className="text-red-600 text-xs mt-1 flex items-center">❌ {fieldErrors.projectCost}</p>}
                          </div>
                          ...
                          <div>
                            ...
                            >
                              ...
                              {Array.from({ length: 20 }, (_, i) => i + 1).map((year) => (
                                <option key={year} value={`${year} years`}>{year} year{year > 1 ? 's' : ''}</option>
                              ))}
                            </select>
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) => {
                                ...
                                if (fieldErrors.employmentType) setFieldErrors({ ...fieldErrors, employmentType: '' });
                              }}
                              className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                                fieldErrors.employmentType ? 'border-red-500 bg-red-50' : 'border-gray-300'
                              }`}
                            >
                            ...
                            {fieldErrors.employmentType && <p className="text-red-600 text-xs mt-1 flex items-center">❌ {fieldErrors.employmentType}</p>}
                          </div>

                          {/* Monthly Income - 6-Tier Dropdown */}
                          <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-2">Monthly Income (Approx.) *</label>
                            <select
                              ...
                              className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                                fieldErrors.monthlyIncome ? 'border-red-500 bg-red-50' : 'border-gray-300'
                              }`}
                            >
                            ...
                            {fieldErrors.monthlyIncome && <p className="text-red-600 text-xs mt-1 flex items-center">❌ {fieldErrors.monthlyIncome}</p>}
                          </div>

                          {/* Existing Loans */}
                          <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-2">Existing Loans? *</label>
                            <select
                              value={formData.existingLoans}
                              onChange={(e) => {
                                setFormData({ ...formData, existingLoans: e.target.value });
                                if (fieldErrors.existingLoans) setFieldErrors({ ...fieldErrors, existingLoans: '' });
                              }}
                              className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                                fieldErrors.existingLoans ? 'border-red-500 bg-red-50' : 'border-gray-300'
                              }`}
                            >
                            ...
                            {fieldErrors.existingLoans && <p className="text-red-600 text-xs mt-1 flex items-center">❌ {fieldErrors.existingLoans}</p>}
                          </div>

                          {/* ITR Filed */}
                          <div>
                            <label className="block text-sm font-semibold text-gray-700 mb-2">ITR Filed in Last 2 Years? *</label>
                            <select
                              value={formData.itrFiled}
                              onChange={(e) => {
                                setFormData({ ...formData, itrFiled: e.target.value });
                                if (fieldErrors.itrFiled) setFieldErrors({ ...fieldErrors, itrFiled: '' });
                              }}
                              className={`w-full px-4 py-3 border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition ${
                                fieldErrors.itrFiled ? 'border-red-500 bg-red-50' : 'border-gray-300'
                              }`}
                            >
                              <option value="">Select option</option>
                              <option value="Yes">Yes</option>
                            ...
                            {fieldErrors.itrFiled && <p className="text-red-600 text-xs mt-1 flex items-center">❌ {fieldErrors.itrFiled}</p>}
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {currentStep === 3 && (
                    <div className="space-y-6">
                      ...
                      <div className="bg-purple-50 rounded-lg p-4 md:p-6 border border-purple-100">
                        <h3 className="text-lg md:text-xl font-bold text-gray-900 mb-4 flex items-center">
                          <span className="text-2xl mr-2">✅</span>
                          Consent & Declaration
                        </h3>
                        <div className="space-y-4">
                          <label className="flex items-start cursor-pointer group">
                            <input
                              ...
                              className="mt-1 h-5 w-5 text-green-600 focus:ring-green-500 border-gray-300 rounded cursor-pointer accent-green-600"
                            />
                            <span className="ml-3 text-sm text-gray-700 group-hover:text-gray-900">
                              I authorize Loanzaar and its lending partners to contact me via call, SMS, or WhatsApp regarding my solar loan inquiry.
                            </span>
                          </label>
                  ...
                  <div className="flex flex-col md:flex-row gap-3 md:gap-4 pt-4 border-t border-gray-200 mt-6">
                    ...
                    {currentStep < 3 ? (
                      <button
                        type="button"
                        onClick={nextStep}
                        className="px-4 md:px-6 py-2 md:py-3 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:shadow-lg transition font-semibold text-sm md:text-base"
                      >
                        Next →
                      </button>
                    ) : (
                      <button
                        ...
                        className="px-4 md:px-6 py-2 md:py-3 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:shadow-lg transition font-semibold text-sm md:text-base disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        Get Solar Loan Quote
                      </button>
                    )}
                  </div>
                </form>
              ...
              <div className="min-h-[400px] flex flex-col items-center justify-center p-4 md:p-8 text-center">
                ...
                <p className="text-gray-600 text-sm md:text-base mb-2">Your solar loan application has been received.</p>
                <p className="text-gray-600 text-sm md:text-base mb-8">Our solar finance team will contact you within 1-2 hours to discuss your options.</p>
                <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6 max-w-sm text-sm text-gray-700">
                  <p className="font-semibold mb-2">What happens next:</p>
                  <ul className="text-left space-y-1 text-xs md:text-sm">
                    <li>✓ Application verification</li>
                    <li>✓ Solar system assessment & quotation</li>
                    <li>✓ Instant approval & fund disbursement</li>
                  </ul>
                </div>
                <button
                  onClick={closeModal}
                  className="px-6 md:px-8 py-2 md:py-3 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg hover:shadow-lg transition font-semibold text-sm md:text-base"
                >
                  Close
                </button>
              </div>
